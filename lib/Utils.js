...
Stravi.Utils = function(game) {
    ...
    this.tileLoaded = function(type, img)
    {
        ...
        this.tileCounter--;
        if (this.tileCounter == 0 && this.preloadCallback !== false)
        {
            this.preloadCallback.call();
        }
    }

    this.getNeighbours = function(x, y) {
        return [{   x: this.cx(y%2==0 ? x : x-1),   y: this.cy(y-1) },
                {   x: this.cx(x-1),                y: y            },
                {   x: this.cx(y%2==0 ? x : x-1),   y: this.cy(y+1) },
                {   x: this.cx(y%2==0 ? x+1 : x),   y: this.cy(y+1) },
                {   x: this.cx(x+1),                y: y            },
                {   x: this.cx(y%2==0 ? x+1 : x),   y: this.cy(y-1) }];
    }
    ...
    /*
     * Methods only available on the server
     */
    if (Meteor.isServer)
    {
        /*
         * Install function, generates all tiles (type: plain)
         */
         ...
         * 'Clears' all tiles (set type: plain), then generates different terrain types based on generation parameters
         */
        this.terrainGenerator = function() {
            ...
            for (type in Stravi._settings.terrain)
            {
                ...
                if (terrain.percentage > 0)
                {
                    ...
                    // Create empty array to store terrain coordinates
                    terrainLocations = [];
                    while (terrainTiles > 0)
                    {
                        ...
                        terrainLocations.push({ 'x': rX, 'y': rY });
                        generated++;
                        ...
                        while (generated < size)
                        {
                            ...
                            {
                                neighbour = nb.shift();
                                terrainLocations.push({'x': neighbour.x, 'y': neighbour.y});
                                generated++;
                            }
                        }
                    ...
                    console.log(terrainLocations);
                    tileCollection.update({$or: terrainLocations}, {$set: {'type': type}}, {'multi': true}, function(e) { console.log(type + ' terrain generated', e); });
                }
            }
