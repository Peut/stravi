...
Stravi.Utils.prototype.terrainGenerator = function() {
    ...
    tileCollection.find().forEach(function(tile) {
        tileCollection.update({'_id': tile._id}, {$set: {'type': 'plain'}}, function(e) { console.log(e); });
    });
        ...
        if (terrain.percentage > 0)
        {
            ...
            while (terrainTiles > 0)
            {
                if (terrainTiles < terrain.maxSize)
                {
                    ...
                    size = Math.ceil(Math.random()*terrain.maxSize);
                }
                ...
                tileCollection.update({'_id': tile[0]._id}, {$set: {'type': type}}, function(e) { console.log(e); });
                
                size--;
                while (size > 0)
                {
                    nb = this.getNeighbours(rX, rY);
                    while (size > 0 && nb.length > 0)
                    {
                        neighbour = nb.shift();
